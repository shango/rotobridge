...
**The ES3 side is tested without After Effects, all of it.** The two core files touch no host and load under plain node, so `node test/test_ae_core.js` runs 70 tests. `test/ae_mock.js` then stands in for the host - `valueAtTime`, `sourcePointToComp` with no time parameter, mask properties by matchName, feather points on the `Shape` object, and an exactly affine layer transform - which lets the adapters themselves run end to end: 45 export tests and 39 import tests, among them an export/import/export round trip that returns every vertex to within 1e-9 px through a layer at scale [150, 220] and 30 degrees, with the whole `shapes` array - `keys` included - comparing byte-equal. `test/run.sh` runs the five host-free suites; 321 tests.
